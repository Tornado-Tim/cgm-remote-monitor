...
function init (client, d3) {
  ...
  var dotRadius = function(type) {
    ...
    if (type === 'mbg') {
      ...
      radius = Math.min(2, radius - 1).toFixed(2);
    }
    ...
    function prepareFocusCircles(sel) {
      ...
      sel.attr('cx', function (d) {
        ...
        .attr('fill', function (d) {
          return d.type === 'forecast' ? d.fill : d.color;
        })
        .attr('fill-opacity', function (d) {
          return d.type === 'forecast' ? chart().futureOpacity(d.mills - client.latestSGV.mills) : null;
        })
        .attr('opacity', function (d) {
          return d.noFade ? null : 0.65;
        })
        .attr('stroke-width', function (d) {
          return d.type === 'mbg' ? 2 : d.type === 'forecast' ? 2 : null;
        })
        .attr('stroke', function (d) {
          return d.type === 'mbg' ? 'white' : d.type === 'forecast' ? d.color : null;
        })
        .attr('r', function (d) {
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles ( ) {
    ...
    function prepareTreatCircles(sel) {
      function strokeColor(d) {
        var color = 'rgba(255, 255, 255, 0.65)';
        if (d.isAnnouncement) {
      ...
      function fillColor(d) {
        var color = 'rgba(128, 128, 128, 0.65)';
        if (d.isAnnouncement) {
          color = 'rgba(255, 165, 0, 0.65)';
        } else if (d.glucose) {
  ...
  renderer.addContextCircles = function addContextCircles ( ) {
    ...
    function prepareContextCircles(sel) {
      ...
      sel.attr('cx', function (d) { return chart().xScale2(new Date(d.mills)); })
        ...
        .attr('stroke-width', function (d) { return d.type === 'mbg' ? 2 : null; })
        .attr('stroke', function (d) { return d.type === 'mbg' ? 'white' : null; })
  ...
  function prepareArc(treatment, radius) {
    var arc_data = [
      ...
      { 'element': '', 'color': 'rgba(255,255,255,0.35)', 'start': -1.5708, 'end': 1.5708, 'inner': 0, 'outer': radius.R1 },
      { 'element': '', 'color': 'transparent', 'start': -1.5708, 'end': 1.5708, 'inner': radius.R2, 'outer': radius.R3 },
      // blue insulin half-circle on bottom
      { 'element': '', 'color': 'rgba(15, 116, 187, 0.5)', 'start': 1.5708, 'end': 4.7124, 'inner': 0, 'outer': radius.R1 },
