...
//console.log(window);
  ...
  sorteddaystoshow.forEach(function (day) {
    if (datastorage[day].netBasalPositive) {
      ...
      for (var h = 0; h < 24; h++) {
        ...
        totalNet[h] += net;
        var color = Math.abs(net) < 0.019 ? "#adabab" : (net < 0 ? "red" : "lightgreen");
        $('<td>' +
          '<span style="color:#adabab;">' + negative.toFixed(2) + '</span>' + '<br>' +
          '<span style="color:#adabab;">' + positive.toFixed(2) + '</span>' + '<br>' +
  ...
  if (days > 0) {
    ...
    for (var h = 0; h < 24; h++) {
      var color = Math.abs(totalNet[h]) < 0.01 ? "transparent" : (totalNet[h] < 0 ? "red" : "lightgreen");
      $('<td style="background-color:' + color + '";>' +
        '<span style="color:#adabab;">' + (totalNegative[h] / days).toFixed(2) + ' (' + negativesCount[h] + ')' + '</span>' + '<br>' +
        '<span style="color:#adabab;">' + (totalPositive[h] / days).toFixed(2) + ' (' + positivesCount[h] + ')' + '</span>' + '<br>' +
        '<span style="color:#adabab;font-weight:bold;">' + (totalNet[h] / days).toFixed(2) + '</span>' +
        '</td>').appendTo(tr);
    }
    table.append(tr);
  }
