    ...
    return [
      {
        ...
        , bg: false, insulin: false, carbs: false, prebolus: false, duration: true, percent: false, absolute: false, profile: false, split: false, units: 'mgdl'
    ...
    function getForecastPoints ( ) {
      ...
      function toPoints (offset, forecastType, fcastcolor) {
        return  function toPoint (value, index) {
          if (value > (client.settings.thresholds.bgHigh*2)) {
            fcastcolorfill = '#e3b4f3';
          } else if (value > (client.settings.thresholds.bgHigh*1.80)) {
            fcastcolorfill = '#b800f5';
          } else if (value > (client.settings.thresholds.bgHigh*1.60)) {
            fcastcolorfill = '#fd20d7';
          } else if (value > (client.settings.thresholds.bgHigh*1.40)) {
            fcastcolorfill = '#f38ee2';
          } else if (value > (client.settings.thresholds.bgHigh*1.20)) {
            fcastcolorfill = '#f76767';
          } else if (value > client.settings.thresholds.bgHigh) {
            fcastcolorfill = 'red';
          } else if (value > (client.settings.thresholds.bgTargetTop*1.25)) {
            fcastcolorfill = 'orange';
          } else if (value > client.settings.thresholds.bgTargetTop) {
            fcastcolorfill = 'yellow';
          } else if (value >= (client.settings.thresholds.bgTargetBottom*1.25) && sgv <= (client.settings.thresholds.bgTargetTop/1.25)) {
            fcastcolorfill = '#3fce02';
          } else if (value >= client.settings.thresholds.bgTargetBottom && sgv <= client.settings.thresholds.bgTargetTop) {
            fcastcolorfill = '#02ad27';
          } else if (value < client.settings.thresholds.bgLow) {
            fcastcolorfill = '#2196f3';
          } else if (value < client.settings.thresholds.bgTargetBottom) {
            fcastcolorfill = '#06bbbb';
          }
          return {
            mgdl: value
            , color: fcastcolor
      ...
      if (prop.lastPredBGs) {
        if (prop.lastPredBGs.values) {
          // very old legacy/unsupported openaps versions use this
          // for compatability reasons it is still here
          points = points.concat(_.map(prop.lastPredBGs.values, toPoints(0, "Values", '#bad2ef')));
        }
        if (prop.lastPredBGs.IOB) {
          points = points.concat(_.map(prop.lastPredBGs.IOB, toPoints(3333, "IOB", '#8407c3')));
        }
        if (prop.lastPredBGs.ZT) {
          points = points.concat(_.map(prop.lastPredBGs.ZT, toPoints(4444, "Zero-Temp", '#5caff1')));
        }
        if (prop.lastPredBGs.aCOB) {
          //aCOB or accelerated COB was removed in oref 0.6
          //used for compatability with old openaps versions
          points = points.concat(_.map(prop.lastPredBGs.aCOB, toPoints(5555, "Accel-COB", '#90c725')));
        }
        if (prop.lastPredBGs.COB) {
          points = points.concat(_.map(prop.lastPredBGs.COB, toPoints(7777, "COB", '#ebcdf9')));
        }
        if (prop.lastPredBGs.UAM) {
          points = points.concat(_.map(prop.lastPredBGs.UAM, toPoints(9999, "UAM", '#f90a6d')));
        }
      }
